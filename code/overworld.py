...
class Overworld:
    def __init__(self, tmx_map, data, overworld_frames):
        ...
        self.current_node = [
            node for node in self.node_sprites if node.level == 0][0]
    ...
    def setup(self, tmx_map, overworld_frames):
        ...
        for obj in tmx_map.get_layer_by_name('Paths'):
            pos = [(int(p.x + TILE_SIZE / 2), int(p.y + TILE_SIZE / 2))
                   for p in obj.points]
        ...
        for obj in tmx_map.get_layer_by_name('Nodes'):
            ...
            if obj.name == 'Node' and obj.properties['stage'] == self.data.current_level:
                self.icon = Icon((obj.x + TILE_SIZE / 2, obj.y + TILE_SIZE / 2),
                                 self.all_sprites,
                                 overworld_frames['icon'])

            # nodes
            if obj.name == 'Node':
                available_paths = {k: v for k, v in obj.properties.items() if k in (
                    'left', 'right', 'up', 'down')}
    ...
    def create_path_sprites(self):
        # get tiles from path
        nodes = {node.level: vector(node.grid_pos)
        ...
        for key, path in path_tiles.items():
            for index, tile in enumerate(path):
                if index > 0 and index < len(path) - 1:
                    ...
                    PathSprite(pos=(tile.x * TILE_SIZE, tile.y * TILE_SIZE),
                               surf=surf,
    ...
    def get_current_node(self):
        nodes = pygame.sprite.spritecollide(
