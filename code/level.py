...
class Level:
    def __init__(self, tmx_map, level_frames, audio_files, data, switch_stage):
        ...
        self.pearl_sound.set_volume(0.1)
    ...
    def setup(self, tmx_map, level_frames, audio_files):
        ...
        for obj in tmx_map.get_layer_by_name('BG details'):
            if obj.name == 'static':
                Sprite((obj.x, obj.y), obj.image,
                       self.all_sprites, z=Z_LAYERS['bg tiles'])
            else:
                AnimatedSprite(
                    (obj.x, obj.y), level_frames[obj.name], self.all_sprites, z=Z_LAYERS['bg tiles'])
                if obj.name == 'candle':
                    AnimatedSprite(
                        (obj.x, obj.y) + vector(-20, -20), level_frames['candle_light'], self.all_sprites, z=Z_LAYERS['bg tiles'])
            ...
            if obj.name == 'player':
                ...
                if obj.name in ('barrel', 'crate'):
                    ...
                    if obj.name == 'floor_spike' and obj.properties['inverted']:
                        frames = [pygame.transform.flip(
                            frame, False, True) for frame in frames]
                    ...
                    # animation speed
                    animation_speed = ANIMATION_SPEED if not 'palm' in obj.name else ANIMATION_SPEED + \
                        uniform(-2, 0)
        ...
        for obj in tmx_map.get_layer_by_name("Moving Objects"):
            if obj.name == 'spike':
                Spike(pos=(obj.x + obj.width / 2, obj.y + obj.height / 2),
                ...
                groups = (self.all_sprites, self.semicollision_sprites) if obj.properties['platform'] else (
                ...
                speed = obj.properties['speed']
                MovingSprite(frames, groups, start_pos,
                             end_pos, move_dir, speed, obj.properties['flip'])

                if obj.name == 'saw':
                    if move_dir == 'x':
                        y = start_pos[1] - \
                            level_frames['saw_chain'].get_height() / 2
                        left, right = int(start_pos[0]), int(end_pos[0])
                        for x in range(left, right, 20):
                            Sprite(
                                (x, y), level_frames['saw_chain'], self.all_sprites, Z_LAYERS['bg details'])
                    else:
                        x = start_pos[0] - \
                            level_frames['saw_chain'].get_width() / 2
    ...
    def pearl_collision(self):
        for sprite in self.collision_sprites:
            sprites = pygame.sprite.spritecollide(
                sprite, self.pearl_sprites, True)
            if sprites:
                ParticleEffectSprite(
                    (sprites[0].rect.center), self.particle_frames, self.all_sprites)
        ...
        for sprite in self.damage_sprites:
            if sprite.rect.colliderect(self.player.hitbox_rect):
                ...
                if hasattr(sprite, 'pearl'):
                    sprite.kill()
                    ParticleEffectSprite(
                        (sprite.rect.center), self.particle_frames, self.all_sprites)
